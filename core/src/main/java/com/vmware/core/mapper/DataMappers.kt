...
class DataMapperException(message: String, throwable: Throwable? = null) : Exception(message, throwable)
...
abstract class DataToDatabaseMapper<INPUT : Any, OUTPUT : Any> {
    ...
    fun toDatabase(input: INPUT): OUTPUT = try {
        ...
        throw DatabaseMapperException("Could not map ${input::class.simpleName} to Database", throwable)
    }
...
abstract class DataToPersistenceMapper<INPUT : Any, OUTPUT : Any> {
    ...
    fun toPersistence(input: INPUT): OUTPUT = try {
        ...
        throw DatabaseMapperException("Could not map ${input::class.simpleName} to Persistence", throwable)
    }
